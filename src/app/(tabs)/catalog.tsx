  ...
  return (
    ...
    >
      <View
        style={[
          styles.thumb,
          { backgroundColor: RANK_BG[item.rank] ?? '#EAF3DE' },
        ]}
      >
      ...
      <View style={styles.rowBody}>
        ...
        <Text style={styles.countText}>
          {item.species_count.toLocaleString()}{' '}
          {t('catalog.species_count').includes('{{count}}')
            ? t('catalog.species_count').replace('{{count}}', '')
            : ''}
        </Text>
      </View>
...
export default function CatalogScreen() {
  ...
  const rankQuery = useTaxaByRank('kingdom', 'en');

  const childrenQuery = useTaxonChildren(
    params.taxon_id ?? '',
    'en',
    !isRoot
  );
  ...
  const breadcrumb: TaxonListItem[] = params.breadcrumb
    ? JSON.parse(params.breadcrumb)
    : [];
  ...
  const currentRankIdx = params.rank
    ? RANK_ORDER.indexOf(params.rank)
    : 0;
      ...
      <View style={styles.rankBar}>
        {RANK_ORDER.map((r, i) => (
          <View
            ...
            style={[
              styles.rankPill,
              i === currentRankIdx && styles.rankPillActive,
            ]}
          >
            <Text
              style={[
                styles.rankPillText,
                i === currentRankIdx && styles.rankPillTextActive,
              ]}
            >
              {t(`rank.${r}`)}
            </Text>
