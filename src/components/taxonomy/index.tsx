  ...
  return (
    ...
    >
      <View
        style={[
          rowStyles.thumb,
          { backgroundColor: RANK_BG[item.rank] },
        ]}
      >
      ...
      <View style={rowStyles.body}>
        ...
        <Text style={rowStyles.latin} numberOfLines={1}>
          {item.scientific_name}
        </Text>

        <View style={rowStyles.meta}>
          {showRank && (
            <Text style={rowStyles.rankLabel}>
              {t(`rank.${item.rank}`)}
            </Text>
          )}
          ...
          {item.species_count > 1 && (
            <Text style={rowStyles.count}>
              {item.species_count.toLocaleString()} {t('catalog.species_count').includes('{{count}}') ? '' : ''}
            </Text>
          )}
        </View>
      </View>
  ...
  return (
    ...
    >
      <TouchableOpacity onPress={onTapRoot}>
        <Text style={bcStyles.item}>
          {t('catalog.all_kingdoms')}
        </Text>
      </TouchableOpacity>
        ...
        <View key={bc.id} style={bcStyles.crumb}>
          ...
          <TouchableOpacity onPress={() => onTapCrumb(bc, i)}>
            <Text
              style={[
                bcStyles.item,
                i === breadcrumb.length - 1 && bcStyles.active,
              ]}
            >
    ...
    >
      {RANK_ORDER.map((rank, i) => (
        <View
          ...
          style={[
            rankStyles.pill,
            i === currentIdx && rankStyles.pillActive,
          ]}
        >
          <Text
            style={[
              rankStyles.text,
              i === currentIdx && rankStyles.textActive,
            ]}
          >
            {t(`rank.${rank}`)}
          </Text>
...
export function IucnBadge({
  ...
  return (
    <View
      style={[iucnStyles.pill, { borderColor: color }]}
    >
      <View
        style={[iucnStyles.dot, { backgroundColor: color }]}
      />
      <Text style={[iucnStyles.text, { color }]}>
        {t(`iucn.${status}`)}
      </Text>
    </View>
  );
...
const iucnStyles = StyleSheet.create({
  pill: {
    ...
    alignItems: 'center',
    gap: 6,
    backgroundColor: 'rgba(241,239,232,0.92)',
    borderWidth: 0.5,
    borderRadius: 20,
    paddingHorizontal: 10,
    paddingVertical: 4,
